...
var Board = function(size) {
	...
	this.add = function(checker, row, col){
		if (this.isEmptyLocation(row,col)){
			...
			this.dispatchBoardEvent("add", details); //change view via index.html
		}
	} 
	...
	this.moveTo = function(checker, toRow, toCol){
		if (this.isEmptyLocation(toRow,toCol)){
			...
			this.dispatchBoardEvent("move", details); //change view via index.html
		}
	}
	...
	this.remove = function(checker) {
		...
		this.dispatchBoardEvent("remove", details); //change view via index.html			 
	}
	...
	this.dispatchBoardEvent = function(type, details){
		...
		if (this.allHandlers[type]){ //checks if type property exists
			for (var i in this.allHandlers[type]){
				this.allHandlers[type][i](newEvent); //This calls the <type> function and passes it parameter newEvent
			}
		}
	}
	...
	this.addEventListener = function(eventType, handler){
		if (!this.allHandlers[eventType]) //if eventType property doesnt exist, create it
